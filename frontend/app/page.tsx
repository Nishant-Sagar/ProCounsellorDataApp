...
import { pascalCase } from './utils/pascalCase';
import {MapPin, Users, Award, ChevronRight, Sparkles, MessageCircle, Smartphone, Star, TrendingUp, Calendar, BookOpen, Download, Zap } from 'lucide-react';
...
export default async function CollegesPage() {
  const res = await fetch('https://procounsellor-backend-1000407154647.asia-south1.run.app/api/colleges/all',{headers:{Accept:'application/json'}});
  const colleges: College[] = await res.json()
   if (!res.ok) {
          throw new Error(`Failed to fetch colleges. Status: ${res.status}`);
        }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="relative mb-12 overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-emerald-600 via-teal-600 to-blue-600 rounded-2xl"></div>
          <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/90 to-teal-600/90 rounded-2xl"></div>
          
          {/* Floating Animation Elements */}
          <div className="absolute top-4 left-8 w-4 h-4 bg-white/20 rounded-full animate-ping"></div>
          <div className="absolute top-8 right-12 w-6 h-6 bg-white/15 rounded-full animate-pulse animation-delay-300"></div>
          <div className="absolute bottom-6 left-16 w-3 h-3 bg-white/25 rounded-full animate-bounce animation-delay-500"></div>
          <div className="absolute bottom-8 right-8 w-5 h-5 bg-white/10 rounded-full animate-pulse animation-delay-700"></div>
          ...
          <div className="relative z-10 p-8 text-center text-white">
            <div className="max-w-4xl mx-auto">
              ...
              <h2 className="text-3xl md:text-4xl font-bold mb-4 animate-fade-in-up animation-delay-200">
                <span className="inline-block animate-text-gradient bg-gradient-to-r from-white via-blue-100 to-white bg-clip-text text-transparent">
                  Get Expert Guidance Instantly!
                </span>
              </h2>
              
              {/* Animated Description */}
              <p className="text-emerald-100 mb-8 max-w-2xl mx-auto leading-relaxed animate-fade-in-up animation-delay-400">
                Download our app to connect with expert counsellors who will help you choose the right college based on your interests, budget, and career goals.
              </p>
              
              {/* Animated Download Buttons */}
              <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up animation-delay-600">
                {/* Google Play Store Button with Hover Animation */}
                <a 
                  href="https://play.google.com/store/apps/details?id=your.app.package" 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="group bg-white text-emerald-600 font-semibold py-4 px-8 rounded-xl hover:bg-gray-50 transition-all duration-300 shadow-lg hover:shadow-2xl flex items-center justify-center gap-3 transform hover:scale-105 hover:-translate-y-1"
                >
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      <div className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full animate-ping group-hover:animate-pulse"></div>
                    </div>
                    <div className="text-left">
                      <div className="text-xs text-emerald-600/70 group-hover:text-emerald-600 transition-colors">Get it on</div>
                      <div className="text-sm font-bold group-hover:text-emerald-700 transition-colors">Google Play</div>
                    </div>
                  </div>
                  <Download className="w-4 h-4 group-hover:animate-bounce animation-delay-200" />
                </a>
                
                {/* App Store Button with Hover Animation */}
                <a 
                  href="https://apps.apple.com/app/your-app-name/idYOUR_APP_ID" 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="group bg-emerald-700/50 backdrop-blur-sm text-white font-semibold py-4 px-8 rounded-xl hover:bg-emerald-700/70 transition-all duration-300 border border-emerald-400/30 flex items-center justify-center gap-3 transform hover:scale-105 hover:-translate-y-1 shadow-lg hover:shadow-2xl"
                >
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <svg className="w-6 h-6 group-hover:animate-bounce" viewBox="0 0 24 24" fill="currentColor">
                      ...
                      <div className="absolute -top-1 -right-1 w-2 h-2 bg-blue-400 rounded-full animate-ping group-hover:animate-pulse"></div>
                    </div>
                    <div className="text-left">
                      <div className="text-xs text-white/70 group-hover:text-white transition-colors">Download on the</div>
                      <div className="text-sm font-bold group-hover:text-blue-100 transition-colors">App Store</div>
                    </div>
                  </div>
                  <Download className="w-4 h-4 group-hover:animate-bounce animation-delay-200" />
                </a>
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-6 text-emerald-200 text-sm animate-fade-in-up animation-delay-800">
                <div className="flex items-center gap-2 group cursor-pointer">
                  <Smartphone className="w-4 h-4 group-hover:animate-bounce" />
                  <span className="group-hover:text-white transition-colors">Free Download</span>
                </div>
                <div className="flex items-center gap-2 group cursor-pointer">
                  <MessageCircle className="w-4 h-4 group-hover:animate-bounce animation-delay-100" />
                  <span className="group-hover:text-white transition-colors">Expert Counsellors</span>
                </div>
                <div className="flex items-center gap-2 group cursor-pointer">
                  <Award className="w-4 h-4 group-hover:animate-bounce animation-delay-200" />
                  <span className="group-hover:text-white transition-colors">Personalized Guidance</span>
                </div>
                <div className="flex items-center gap-2 group cursor-pointer">
                  <Zap className="w-4 h-4 group-hover:animate-bounce animation-delay-300" />
                  <span className="group-hover:text-white transition-colors">Instant Support</span>
                </div>
              </div>
            </div>
          </div>
