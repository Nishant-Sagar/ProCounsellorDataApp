...
export default function CourseStats({ data }: CourseStatsProps) {
  ...
  branches.forEach((branch) => {
    ...
    if (Array.isArray(branch.popularColleges)) {
      branch.popularColleges.forEach((college) => totalColleges.add(college));
    }
    if (Array.isArray(branch.jobRoles)) {
    ...
    if (Array.isArray(branch.examId)) {
      ...
      allExams.add(branch.examId);
    }
  });

  const feeRanges = branches
    .map((branch) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl p-6 text-white text-center">
        ...
        <h3 className="text-xl font-bold mb-2">Start Your {data.courseName} Journey</h3>
        <p className="text-green-100 mb-4">
          Explore colleges and specializations in {data.courseName}
        </p>
        <div className="space-y-2">
          <button className="bg-white cursor-pointer text-green-600 font-semibold px-6 py-2 rounded-lg hover:bg-gray-50 transition-colors w-full">
            Find Colleges
          </button>
          <button className="bg-green-500 cursor-pointer text-white font-semibold px-6 py-2 rounded-lg hover:bg-green-400 transition-colors w-full border border-green-400">
            Download Brochure
          </button>
        </div>
      </div>
