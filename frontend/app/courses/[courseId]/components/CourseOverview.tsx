...
export default function CourseOverview({ data }: CourseOverviewProps) {
  // Calculate average salary from all branches
  const salaryRanges = data.branches?.map(branch => 
  ...
  const highestSalaries = data.branches?.map(branch => 
    parseFloat(branch.placementStats?.highestSalary2025?.replace(/[^\d.]/g, '') || '0')
  ).filter(salary => salary > 0);
  
  const maxSalary = highestSalaries?.length > 0 ? Math.max(...highestSalaries) : 'N/A';
    ...
    <div className="bg-white rounded-xl p-8 shadow-lg border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 rounded-lg p-6 border border-purple-100">
          ...
          <div className="text-2xl font-bold text-purple-800">{data.branches?.length || 0}</div>
